...
async function getGame(res) {
	const rawgResponse = await fetch('https://api.rawg.io/api/games?key=' + RAWG_API_KEY);
	...
	res.write(`data: { "name": "${randomGame.name}", "metacritic": ${randomGame.metacritic}, "playtime": ${randomGame.playtime} }\n\n`)
}
